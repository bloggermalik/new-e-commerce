...
export function SingleProductPage({
  ...
  // --- FIX: Enhanced Swipe Configuration ---
  const handlers = useSwipeable({
    onSwipedLeft: handleNext,
    onSwipedRight: handlePrev,
    trackMouse: true,       // Allows testing on desktop with mouse
    preventScrollOnSwipe: true, // Prevents scrolling page when swiping carousel
    delta: 10,              // Minimum distance to trigger a swipe
  });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 px-4 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col items-center w-full space-y-4">
        ...
        {/* --- FIX: Added touch-pan-y class --- */}
        <div
          {...handlers}
          className="relative w-full aspect-square bg-gray-100 rounded-2xl overflow-hidden touch-pan-y cursor-grab active:cursor-grabbing"
        >
          <div
            className="flex h-full w-full transition-transform duration-300 ease-in-out"
            style={{ transform: `translateX(-${currentIndex * 100}%)` }}
          >
            {images.map((img, i) => (
              <div key={i} className="relative w-full h-full shrink-0 select-none">
                <Image
                  src={img}
                  alt={`Image ${i}`}
                  fill
                  className="object-contain"
                  // --- FIX: Disable default browser image dragging ---
                  draggable={false}
          ...
          {/* Optional: Visual Indicator for multiple images */}
          {images.length > 1 && (
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
               {images.map((_, i) => (
                 <div 
                   key={i} 
                   className={`h-2 w-2 rounded-full transition-colors ${i === currentIndex ? 'bg-primary' : 'bg-gray-300'}`}
                 />
               ))}
             </div>
          )}
            ...
            {images.map((img, i) => (
              <button
                ...
                className={`relative w-16 h-16 rounded-md overflow-hidden border-2 ${
                  currentIndex === i // Use currentIndex for active state logic
                    ? "border-primary"
                    : "border-transparent"
                }`}
              >
      ...
      <div className="flex flex-col space-y-2 text-gray-900 font-sans">
        ...
        <div className="">
          <h2 className="text-lg font-medium mb-3">Customer Reviews</h2>
          <div className="overflow-y-auto space-y-4 pr-2 max-h-[400px]">
            {/* Loading state */}
            {isLoading && (
              <div className="flex justify-center">
                <Loader2 className="h-6 w-6 animate-spin text-blue-500" />
